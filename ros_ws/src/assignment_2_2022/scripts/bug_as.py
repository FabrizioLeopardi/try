...
def clbk_laser(msg):
    global regions_
    regions_ = {
        #'fleft':  min(min(msg.ranges[0:143]), 10),
        #'left': min(min(msg.ranges[144:287]), 10),
        #'right':  min(min(msg.ranges[288:431]), 10),
        #'fright':  min(min(msg.ranges[432:575]), 10),
        #'front':   min(min(msg.ranges[576:719]), 10),
        
        'front':   min(min(msg.ranges[0:72]),min(msg.ranges[646:713]), 10),
        'fleft':  min(min(msg.ranges[73:216]), 10),
        'left': min(min(msg.ranges[217:359]), 10),
        'right':  min(min(msg.ranges[360:502]), 10),
        'fright':  min(min(msg.ranges[503:645]), 10),
...
def planning(goal):
    ...
    while not rospy.is_shutdown():
        ...
        if act_s.is_preempt_requested():
            ...
            act_s.publish_feedback(feedback)
            #rospy.loginfo('r')
            #rospy.loginfo(regions_['right'])
            #rospy.loginfo('fr')
            #rospy.loginfo(regions_['fright'])
            #rospy.loginfo('f')
            #rospy.loginfo(regions_['front'])
            #rospy.loginfo('fl')
            #rospy.loginfo(regions_['fleft'])
            #rospy.loginfo('l')
            #rospy.loginfo(regions_['left'])
            #rospy.loginfo('/////////////////////')
            #rospy.loginfo('')
            if regions_['front'] < 0.8:
                change_state(1)
        elif state_ == 1:
